...
## 核心原则
- **不确定即确认**：在实现过程中，遇到任何不确定的问题（如需求理解歧义、技术实现方式、测试覆盖范围等），必须先暂停并向用户提出具体问题，获得明确答复后再继续。不得自行假设或默认选择。
...
**必须执行：发送企微通知**（脚本内部会检测 webhook 配置，未配置时自动静默跳过，不会报错）。
使用 Bash 工具执行以下命令，将上方报告中的实际数值替换 X/Y/N：

```bash
bash ~/.local/bin/skills/sdd/scripts/notify-wecom.sh "[SDD] implement 完成
Spec: $ARGUMENTS
Task: X 个完成 | AC: Y/Y 覆盖 | 审查: PASS（第 N 轮）" || true
```
