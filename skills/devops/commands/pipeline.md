...
你熟悉 Bilibili EP 的 CI/CD 平台，擅长查询流水线状态和诊断构建问题。
...
```bash
curl -s "https://agileflow.bilibili.co/ep/admin/agileflow/v2/ep-auth?\
...
curl -s -H "Access-Token: ${TOKEN}" -H "Perm-Code: ${PERM_CODE}" \
  "https://agileflow.bilibili.co/ep/admin/agileflow/open/application/check?appid=<appid>"
```
...
```bash
curl -s -H "Access-Token: ${TOKEN}" -H "Perm-Code: ${PERM_CODE}" \
  "https://agileflow.bilibili.co/ep/admin/agileflow/open/pipeline/list?appid=<appid>&ps=20&pn=1"
```
...
```bash
curl -s -H "Access-Token: ${TOKEN}" -H "Perm-Code: ${PERM_CODE}" \
  "https://agileflow.bilibili.co/ep/admin/agileflow/open/pipeline/detail?pipeline_id=<pipeline_id>"
```
...
```bash
curl -s -H "Access-Token: ${TOKEN}" -H "Perm-Code: ${PERM_CODE}" \
  "https://agileflow.bilibili.co/ep/admin/agileflow/open/pipeline/config/list?appid=<appid>"
```
...
# 1. 取最新一条
curl -s -H "Access-Token: ${TOKEN}" -H "Perm-Code: ${PERM_CODE}" \
  "https://agileflow.bilibili.co/ep/admin/agileflow/open/pipeline/list?\
appid=<appid>&event_type=5&ps=1&pn=1"

# 2. 查详情
curl -s -H "Access-Token: ${TOKEN}" -H "Perm-Code: ${PERM_CODE}" \
  "https://agileflow.bilibili.co/ep/admin/agileflow/open/pipeline/detail?\
